import ItemCount from "./ItemCount";
import React from 'react';
import { useState, useEffect } from 'react';
import { useContext } from 'react';
import './ItemDetail.css';
import { Link } from 'react-router-dom';
import { useCart } from './Context/CartContext';

function ItemDetail(props) {
    const propsLocal = props;


    const carrito = useCart("");


    const [contador, setContador] = useState(0);


    class itemNuevo {
        constructor(id, nombre, precio, disponible, URL, cantidad) {
            this.id = id;
            this.name = nombre;
            this.price = precio;
            this.stock = disponible;
            this.link = URL;
            this.quantity = cantidad;

        }
    }


    let imagen = `/${props.URL}`;


    const onAdd = (input) => {

        setContador(input);
        const itemPrueba = new itemNuevo(props.id, props.Nombre, props.Precio, props.Stock, props.URL, input)

        carrito.addItem(itemPrueba, input);
    }

    const handleChange = event => {
        //se coloca para evitar el error de consola
    }

    return (

        <div id="container-detalles1" className="container">
            <div id="container-detalles2" className="col-lg-8 border p-3 main-section bg-white">
                <div id="container-detalles2-titulo" className="row hedding m-0 pl-3 pt-0 pb-3">
                    <h2 className="detalle-titulo1">Detalle del Producto</h2>
                </div>
                <div id="filaunica" className="row mb-4">
                    <div id="columnaimagen" className="col-md-4 left-side-product-box pb-3">
                        <img src={imagen} className="border p-3" />
                        <span className="sub-img">
                            {/* <img src="" className="border p-2" />
                            <img src="" className="border p-2" />
                            <img src="" className="border p-2" /> */}
                        </span>
                    </div>
                    <div id="columnamacrodetalles" className="col-lg-8">
                        <div id="columnamacrodetalles-divinterno" className="right-side-pro-detail border p-3 m-0">
                            <div className="row">
                                <div id="filanombre" className="col-lg-12">
                                    <span></span>
                                    <p id="detalle-titulo2" className="m-0 p-0">
                                        {props.Nombre}
                                    </p>
                                </div>
                                <div id="filaprecio" className="col-lg-12">
                                    <p className="m-0 p-0 price-pro">
                                        {props.Precio}$
                                    </p>
                                    <hr className="p-0 m-0" />
                                </div>
                                <div id="filadetalles" className="col-lg-12 pt-2">
                                    <h5>Detalles del producto</h5>
                                    <span>
                                        {props.Descripcion}
                                    </span>
                                    <hr className="m-0 pt-2 mt-2" />
                                </div>
                                <div id="filatipo" className="col-lg-12">
                                    <p className="tag-section"><strong>Tipo : </strong>
                                        <a href="">
                                            {props.Tipo}
                                        </a>
                                    </p>
                                </div>
                                <div id="filastock" className="col-lg-12">
                                    <h6>Stock :</h6>
                                    <input onChange={handleChange} type="number" className="form-control text-center w-100" value={props.Stock} />
                                    {/* <!-- el value deberia ser props.Stock --> */}
                                </div>
                                <div id="filaitemcount" className="col-lg-12 mt-3">
                                    {/* Tengo que arreglar que abajo aparezca un error si se inteta añadir mas que el stock de acuerdo a la sumatoria de CartContext */}
                                    {contador === 0 ? <ItemCount onAdd={onAdd} stock={props.Stock} nombre={props.Nombre} /> : <Link className="btn btn-success" to='/cart'><h6>Añadiste {contador} Items,</h6> <h2>¡Ir al carrito!</h2></Link>}

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>


    );

}

export default ItemDetail;